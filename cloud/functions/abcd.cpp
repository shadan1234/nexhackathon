...
void loop() {
  ...
  String documentPath = "Esp32/GDB-C1";
  ...
  float dist = distance();
  float location = 1; // Dummy location data
  float time = 0; // Dummy time data
  ...
  if (!isnan(dist) && !isnan(location) && !isnan(time)) {
    // Set the fields in the FirebaseJson object
    content.set("fields/dist/stringValue", String(dist, 2));
    content.set("fields/location/stringValue", String(location, 2));
    content.set("fields/time/stringValue", String(time, 2));
    ...
    if (Firebase.Firestore.patchDocument(&fbdo, FIREBASE_PROJECT_ID, "", documentPath.c_str(), content.raw(), "dist,location,time,timestamp")) {
      Serial.printf("ok\n%s\n\n", fbdo.payload().c_str());
    } else {
